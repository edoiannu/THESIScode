...
process = "hed_" + instate + "_eta" + str(eta) + "_alpha" + str(alpha_over_kappa)    # process name
...
# photo-current
def dy_t(rho):
    dW_x = rnd.gauss(0, np.sqrt(dt))
    dW_y = rnd.gauss(0, np.sqrt(dt))
    dW_t = (dW_x + 1j * dW_y) / np.sqrt(2)
    return np.sqrt(eta) * ((cops + cops.dag()) * rho).tr() * dt + dW_t
...
def smesolve_kraus(rho_start):
    ...
    for i in range(1, NUMBER_OF_TIMEINTERVALS):
        kraus = qeye(DIMH) - 1j * HS(alpha_over_kappa) * dt - 0.5 * cops.dag() * cops * dt + np.sqrt(eta) * cops * dy_t(rhot)
        num = kraus * rhot * kraus.dag() + (1 - eta) * cops * rhot * cops.dag() * dt
