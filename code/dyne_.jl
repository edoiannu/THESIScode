...
if det_type == "hod"
    # string that identifies the detection type
    measure = "hod" * string(ϕ_val)
    process = measure * "_" * inputstring
else
    measure = "hed_"
    process = measure * inputstring
end
...
# --- mean values -------------------------------------------------------------
open(processpath * "erg_" * measure * ".dat", "w") do io
    for (t, erg) in zip(tlist, erg_mean)
        @printf(io, "%.3f\t%.8f\n", t, erg)
    end
end
open(processpath * "cap_" * measure * ".dat", "w") do io
    for (t, cap) in zip(tlist, cap_mean)
        @printf(io, "%.3f\t%.8f\n", t, cap)
    end
end
# --- variances ---------------------------------------------------------------
open(processpath * "var_erg_" * measure * ".dat", "w") do io
    for (t, ergvar) in zip(tlist, erg_var)
        @printf(io, "%.3f\t%.8f\n", t, ergvar)
    end
end
open(processpath * "var_cap_" * measure * ".dat", "w") do io
    for (t, capvar) in zip(tlist, cap_var)
        @printf(io, "%.3f\t%.8f\n", t, capvar)
    end
end
# --- skewnesses --------------------------------------------------------------
open(processpath * "skw_erg_" * measure * ".dat", "w") do io
    for (t, ergskw) in zip(tlist, erg_skw)
        @printf(io, "%.3f\t%.8f\n", t, ergskw)
    end
end
open(processpath * "skw_cap_" * measure * ".dat", "w") do io
...
for (i_t, t) in enumerate(target_times)
    open(processpath * "histo_erg_" * measure * "_t$(t).dat", "w") do io
        for val in prog_erg_histo[i_t]
            @printf(io, "%.8f\n", val)
        end
    end
    open(processpath * "histo_cap_" * measure * "_t$(t).dat", "w") do io
