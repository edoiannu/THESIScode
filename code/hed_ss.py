...
# === STEADY STATES DYNAMICS OF A SYSTEM SUBJECT TO A CONTINOUS HETERODYNE DETECTION ===
...
# we pass phi and eta as arguments of command line
phi = int(sys.argv[1])
...
process = "hod" + str(phi) + "_eta" + str(eta)                               # process name
...
def ss_smesolve(alpha_over_kappa):
...
with ProcessPoolExecutor(max_workers = num_cores) as executor:
    for a in alphalist:
        ...
        steadystates = list(executor.map(ss_smesolve, [a] * (NUMBER_OF_TRAJECTORIES)))
