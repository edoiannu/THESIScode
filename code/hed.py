...
# === DYNAMICS OF A SYSTEM SUBJECT TO A CONTINOUS HETERODYNE DETECTION ===
...
for i in range(0, NUMBER_OF_TRAJECTORIES, chunk_dim):
    ...
    hed = smesolve(
        ...
        rho0,
        tlist,
        c_ops = [np.sqrt(1 - eta) * sigmam()],
        sc_ops = [np.sqrt(eta) * sigmam()],
        ntraj = chunk_dim,
        heterodyne = True,
    ...
    with open("states/" + process + "_chunk" + str(chunk_num) + ".pkl", "ab") as f:
        pickle.dump(hed.states, f)  # we print on a file the states evolutions of each chunk
    del hed              # remove reference to solver result to free memory
